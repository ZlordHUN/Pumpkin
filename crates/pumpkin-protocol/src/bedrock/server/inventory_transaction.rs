...
mod tests {
    ...
    fn decodes_use_item_transaction_with_empty_hand() {
        let payload = [
            ...
            0x03, 0x00, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x8c, 0xf6, 0x9b,
            0x43, 0x72, 0x3d, 0x83, 0x42, 0xf3, 0xe1, 0xd1, 0xc3, 0x00, 0x90, 0x61, 0x3f, 0x00,
            0x8d, 0x26, 0x3f, 0x00, 0x00, 0x80, 0x3f, 0xfd, 0x59, 0x01, 0x00,
        ];
        ...
        let mut reader = &payload[6..];
        let input = crate::bedrock::server::player_auth_input::PlayerUseItemTransactionData::read(
            &mut reader,
        )
        .unwrap();
        assert_eq!(input.hand, data.hand);
        assert_eq!(input.item_in_hand.id, data.item_in_hand.id);
        assert_eq!(input.block_runtime_id, data.block_runtime_id);
        assert!(reader.is_empty());
    }
    ...
    fn decodes_use_item_transaction_with_crafting_table() {
        let payload = [
            ...
            0xa6, 0x05, 0x7e, 0xd9, 0x06, 0x01, 0x04, 0x00, 0x3a, 0x00, 0x01, 0x00, 0x00, 0x00,
            0xfd, 0x59, 0x0a, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x5f,
            0x0f, 0xaa, 0x43, 0x72, 0x3d, 0x83, 0x42, 0xb8, 0x39, 0xd5, 0xc3, 0x00, 0x16, 0x15,
            0x3f, 0x00, 0x00, 0x80, 0x3f, 0x00, 0xc8, 0x81, 0x3e, 0xb6, 0x5e, 0x01, 0x00,
        ];
        let mut reader = payload.as_slice();
        let packet = SInventoryTransaction::read(&mut reader).unwrap();
        let TransactionData::UseItem(data) = packet.transaction_data else {
